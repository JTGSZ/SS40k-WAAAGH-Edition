...
/datum/job_quest/slaanesh_champion/main_body()
	switch(alignment)
		...
		if(-6)
			...
			for(var/obj/effect/landmark/bedguitar/BG in range(2, our_protagonist))
				to_chat(our_protagonist, "<span class='notice'>Yes... This is the right place.</span>")
				new /obj/item/weapon/propguitar/one(get_turf(BG))
				alignment--
		if(-7)
			if(our_protagonist.find_held_item_by_type(/obj/item/weapon/propguitar))
		...
		if(-8)
			if(our_protagonist.find_held_item_by_type(/obj/item/weapon/propguitar/five))
		...
		if(-14)
			for(var/obj/effect/landmark/chaosladder/theladder in orange(4,our_protagonist.loc))
				new /obj/structure/ladder/chaosruin/ground(theladder.loc) //We make a new ladder
