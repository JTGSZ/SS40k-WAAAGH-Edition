...
/obj/item/weapon/gun/siren/afterattack(atom/A , mob/living/user as mob|obj, flag, params, struggle = 0)
	if(flag)
...
/obj/item/weapon/gun/siren/supersoaker/afterattack(atom/A , mob/living/user as mob|obj, flag, params, struggle = 0)
	if(flag)
		return //we're placing gun on a table or in backpack
	if(harm_labeled >= min_harm_label)
		to_chat(user, "<span class='warning'>A label sticks the trigger to the trigger guard!</span>")//Such a new feature, the player might not know what's wrong if it doesn't tell them.
