...
/obj/item/weapon/subspacetunneler/afterattack(atom/target , mob/living/user )
	if (target.loc == user)
		return
...
/obj/item/weapon/subspacetunneler/proc/send(turf/T , mob/living/user )
	if(!T)
...
/obj/item/weapon/subspacetunneler/proc/receive(obj/O , mob/living/user , params, reflex = 0)
	if(!loaded_crystal)
		return
