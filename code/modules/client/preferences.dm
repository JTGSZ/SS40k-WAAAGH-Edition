...
/datum/preferences/proc/SetChoices(mob/user, limit = 17, list/splitJobs = list("General", "AI"), widthPerColumn = 360, height = 620)
	...
	switch(alternate_option)
		...
		if(BE_PEASANT)
			HTML += "<center><br><a href='?_src_=prefs;preference=job;task=random'>Be peasant if preference unavailable</a></center><br>"
		if(RETURN_TO_LOBBY)
	...
	if(href_list["preference"] == "job")
		switch(href_list["task"])
			...
			if("reset")
				...
				SetChoices(user)
			if("random")
				if(alternate_option == GET_RANDOM_JOB || alternate_option == BE_PEASANT)
					alternate_option += 1
