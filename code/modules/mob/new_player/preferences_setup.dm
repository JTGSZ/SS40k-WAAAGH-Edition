	...
	if(!for_observer)
		...
		if(job_civilian_high)//I hate how this looks, but there's no reason to go through this switch if it's empty
			...
			switch(job_engsec_high)
				if(GENERAL)
					clothes_s = new /icon(uniform_dmi, "general")
				...
				if(COMMISSAR)
					clothes_s = new /icon(uniform_dmi, "commissar")
					clothes_s.Blend(new /icon(feet_dmi, "noble-boots"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon('icons/mob/hands.dmi', "black"), ICON_UNDERLAY)
				...
				if(IGSERGEANT)
					clothes_s = new /icon(uniform_dmi, "guardsman_under")
				...
				if(INQUISITOR)
					clothes_s = new /icon(uniform_dmi, "uni-church")
					clothes_s.Blend(new /icon(feet_dmi, "noble-boots"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon('icons/mob/hands.dmi', "black"), ICON_UNDERLAY)
