		...
		if(job_civilian_high)//I hate how this looks, but there's no reason to go through this switch if it's empty
			...
			switch(job_engsec_high)
				if(GENERAL)
					...
					clothes_s.Blend(new /icon(feet_dmi, "flakboots"), ICON_UNDERLAY)
				...
				if(IGSERGEANT)
					...
					clothes_s.Blend(new /icon(suit_dmi, "cadia_flak_armor"), ICON_OVERLAY)
				...
				if(IGWEPSPEC)
					...
					clothes_s.Blend(new /icon('icons/mob/head.dmi', "cadian_flak_helmet"), ICON_OVERLAY)
					clothes_s.Blend(new /icon(suit_dmi, "cadia_flak_armor"), ICON_OVERLAY)
					clothes_s=blend_backpack(clothes_s,backbag,"powerpack","powerpack","powerpack")
				if(PRIMARISPSYKER)
					clothes_s = new /icon(uniform_dmi, "chapblack_s")
					clothes_s.Blend(new /icon(feet_dmi, "noble-boots"), ICON_UNDERLAY)
					...
					clothes_s.Blend(new /icon('icons/mob/back.dmi', "psykerstaff"), ICON_OVERLAY)
